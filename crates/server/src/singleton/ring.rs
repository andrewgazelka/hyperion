...
use crate::net::{encoder::DataWriteInfo, RING_SIZE};

// todo: see if it makes sense to use MaybeUninit
#[derive(Debug, Copy, Clone)]
pub struct Ring<const N: usize = RING_SIZE> {
    data: [u8; N],
    head: usize,
}
...
impl<const N: usize> Buf for Ring<N> {
    type Output = DataWriteInfo;

    fn get_contiguous(&mut self, len: usize) -> &mut [u8] {
        debug_assert!(
            len <= N,
            "requested contiguous length of {len} exceeds max_len of {N}",
        );
