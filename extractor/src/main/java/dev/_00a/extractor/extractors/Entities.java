...
import dev._00a.extractor.ClassComparator;
import dev._00a.extractor.DummyPlayerEntity;
import dev._00a.extractor.DummyWorld;
import dev._00a.extractor.Main;
import dev._00a.extractor.Main.Pair;
...
public class Entities implements Main.Extractor {
    ...
    public void extract(MinecraftServer server, DataOutput output, Gson gson)
        ...
        for (var entityClass : Entities.ENTITY_CLASS_TO_TYPE_MAP.keySet()) {
            handleEntity(entityClass, entitiesJson);
        }
    ...
    private void handleEntity(
        ...
        if (entityType != null) {
            ...
            var defaults = (DataTracker.Entry<?>[]) dataTrackerEntriesField.get(dataTracker);

            for (var entry : defaults) {
                var fieldJson = new JsonObject();
                var trackedData = entry.getData();
            ...
            if (entityInstance instanceof LivingEntity) {
                ...
                if (defaultAttributes != null) {
                    ...
                    for (var instance : instances.values()) {
                        var attribute = Registries.ATTRIBUTE.get(instance.getAttribute().getKey().get());
        ...
        for (var entityField : entityClass.getDeclaredFields()) {
            if (entityField.getType().equals(TrackedData.class)) {
                ...
                fieldJson.addProperty("type", Entities.trackedDataHandlerName(trackedData.dataType()));
    ...
    @SuppressWarnings("unchecked")
    private Pair<String, JsonElement> trackedDataToJson(TrackedData<?> data, DataTracker tracker) {
        final var val = tracker.get(data);

        String name = trackedDataHandlerName(data.dataType());
        ...
        switch (name) {
            ...
            case "particle_list": {
                ...
                for (var particle : (List<ParticleEffect>) val) {
                    array.add(Registries.PARTICLE_TYPE.getId(particle.getType()).getPath());
                }
                value = array;
                break;
            }
            ...
            case "entity_pose":
                ...
                break;
            case "cat_variant": {
                var id = ((RegistryEntry<CatVariant>) val).getKey().get().getValue().getPath();
                value = new JsonPrimitive(id);
                break;
            }
            case "wolf_variant": {
                var id = ((RegistryEntry<WolfVariant>) val).getKey().get().getValue().getPath();
                value = new JsonPrimitive(id);
                break;
            }
            case "frog_variant": {
                var id = ((RegistryEntry<FrogVariant>) val).getKey().get().getValue().getPath();
                value = new JsonPrimitive(id);
                break;
            }
            case "painting_variant": {
                var id = ((RegistryEntry<PaintingVariant>) val).getKey().get().getValue().getPath();
                value = new JsonPrimitive(id);
                break;
            }
            ...
            case "armadillo_state": {
                var state = (ArmadilloEntity.State) val;
                value = new JsonPrimitive(state.name());
                break;
            }
            default:
