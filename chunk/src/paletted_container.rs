    ...
    PalettedContainer<T, LEN, HALF_LEN>
{
    ...
    ///
    /// - **`writer`**: The [`Write`] instance to write the paletted container to.
    /// - **`to_bits`**: A function to convert the element type to bits. The output must be less
    ///   than two to the power of `direct_bits`.
    /// - **`min_indirect_bits`**: The minimum number of bits used to represent the element type in
    ///   the indirect representation. If the bits per index is lower, it will be rounded up to
    ///   this.
    /// - **`max_indirect_bits`**: The maximum number of bits per element allowed in the indirect
    ///   representation. Any higher than this will force conversion to the direct representation
    ///   while encoding.
    /// - **`direct_bits`**: The minimum number of bits required to represent all instances of the
    ///   element type. If `N` is the total number of possible values, then `DIRECT_BITS` is
    ///   `floor(log2(N - 1)) + 1`.
...
fn encode_compact_u64s(
    ...
    loop {
        ...
        for i in 0..vals_per_u64 {
            match vals.next() {
                Some(val) => {
                    debug_assert!(val < 2u128.pow(bits_per_val as _) as _);
