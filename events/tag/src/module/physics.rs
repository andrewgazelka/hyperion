...
use hyperion::{
    net::{Compose, ConnectionId, DataBundle}, simulation::{
        animation::ActiveAnimation, blocks::{self, Blocks}, entity_kind::EntityKind, handlers::try_change_position, metadata::entity::Pose, EntitySize, Pitch, Position, Velocity, Yaw
...
impl Module for PhysicsModule {
    fn module(world: &World) {
        ...
        .each_iter(
            |it, row, (compose, prev_position, position, velocity, yaw, pitch, kind, animation)| {
                let world = it.system().world();
                ...
                if velocity.0 != Vec3::ZERO {
                    ...
                    velocity.0 *= 0.99;
                    // check if the player is on the ground by checking the previous position and the current position
                ...
                // velocity: velocity.to_packet_units(),
                // };
                // data_bundle.add_packet(&vel_packet).unwrap();
